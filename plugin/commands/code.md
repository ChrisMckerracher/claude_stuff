...
4. **Message QA teammate in parallel:**
   ```
   Message QA teammate: "Generate tests for task <task-id>.
   Design doc: docs/plans/architect/<feature-name>.md
   Write tests first, Coding Agent will implement."
   ```

### Implementation (TDD)

5. Follow TDD workflow:
   - Write failing test first (coordinate with QA teammate's tests)
...
7. Message Code Review teammate when ready:
   ```
...
8. Verify all tests pass (yours + QA teammate's)
9. Message lead for pre-commit gate: "Ready to commit?"
10. When approved, mark task done with `bd close <task-id>`
...
- Message lead if architecture questions arise
